...
if [ "${WPT_INTERACTIVE,,}" == 'y' ]; then
    ...
    until git clone --depth 1 --branch=$WPT_BRANCH https://github.com/DmitriyStoyanov/WebPageTest.agent wptagent
    do
        sleep 1
    done
fi
...
until sudo apt -y install python3 python3-pip python3-ujson \
        ...
        ttf-mscorefonts-installer fonts-noto fonts-roboto fonts-open-sans upower
do
    sleep 1
done
